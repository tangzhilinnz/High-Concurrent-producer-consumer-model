...
public:
	void* TzAllocSmall(size_t size/*, unsigned int align*/);
...
TzMemPool<__spansize, __spannum>::TzCreatePool() 
{
	...
	__Free_pool_size1 = 400;
	u_char* p_begin1 = __Head;
	for (int i = 0; i < __Free_pool_size1; i++)
	{
		...
		u_char* pchunk = nullptr;
		for (int j = 0; j < 2048; j++)
		{
			*reinterpret_cast<u_char**>(p_begin1) = pchunk;
			pchunk = p_begin1;
			p_begin1 += 256;
		}
		*reinterpret_cast<u_char**>(pchunk + sizeof(u_char*)) = __Free_pool1;
		__Free_pool1 = pchunk;
	}
...
template<SPAN_SIZE_E __spansize, SPAN_NUM_E __spannum>
void* 
TzMemPool<__spansize, __spannum>::TzAllocSmall(size_t size/*, unsigned int align*/)
{
	...
	if (tc->_Tcurrent1 == nullptr)
	{
		//printf("YES\n");
		if (tc->_Tdeposit_size1 != 0)
		{
			tc->_Tcurrent1 = tc->_Tdeposit1;
			tc->_Tdeposit1 = nullptr;
			tc->_Tdeposit_size1 = 0;
			//printf("YES1--tid%d\n", pthread_self());
		}
		else
		{
			...
			//LockGuard lock(&__pool_mutex);
			//if (__Free_pool != nullptr)
			if (__Free_pool_size1 != 0)
			{
				tc->_Tcurrent1 = __Free_pool1;
				//__Free_pool1 = *reinterpret_cast<u_char**>(__Free_pool1 + 8);
				__Free_pool1 = *((u_char**)(__Free_pool1 + 8));
				--__Free_pool_size1;
			}
			else
			{
	...
	u_char* m = tc->_Tcurrent1;
	//tc->_Tcurrent1 = *reinterpret_cast<u_char**>(tc->_Tcurrent1);
	tc->_Tcurrent1 = *((u_char**)tc->_Tcurrent1);
	return m;
}
...
TzMemPool<__spansize, __spannum>::TzFree(void* p)
{
	...
	THREAD_CACHE_S* tc = __threadlocal_data;
	if (tc->_Tdeposit_size1 < 2048)
	{
		*((u_char**)p) = tc->_Tdeposit1;
		tc->_Tdeposit1 = (u_char*)p;
		//*reinterpret_cast<u_char**>(p) = tc->_Tdeposit1;
		//tc->_Tdeposit1 = reinterpret_cast<u_char*>(p);
		++tc->_Tdeposit_size1;
		//printf("yes\n");
	}
	else // tc->_pooldeposit_size1== 2048
	{
		{
			LockGuardSpin lock(&__pool_spin);
			*((u_char**)(tc->_Tdeposit1 + 8)) = __Free_pool1;
			//*reinterpret_cast<u_char**>(tc->_Tdeposit1 + 8) = __Free_pool1;
			__Free_pool1 = tc->_Tdeposit1;
			++__Free_pool_size1;
		}

		*reinterpret_cast<u_char**>(p) = nullptr;
		tc->_Tdeposit1 = reinterpret_cast<u_char*>(p);
		tc->_Tdeposit_size1 = 1;
...
TzMemPool<__spansize, __spannum>::TzDeleteThreadCache(void* ptr)
{
	...
	//CLockGuardSpin lock(&__pool_spin);
