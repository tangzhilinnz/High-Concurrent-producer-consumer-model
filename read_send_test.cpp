...
#define VER            2  /* modify VER to choose the version here */
...
//
//#if (VER == MY_MALLOC)
//			//psend = (char*)mempool->TzAllocSmall(random);
//			psend = (char*)mempool->TzAllocSmall(random) + 8;
//#endif
//#if (VER == TC_MALLOC)
//			psend = (char*)malloc(random) + 8;
//#endif
...
//				--readQueueSize;
//
//				if (quickFetchMode && readQueueSize < 1000)
//				{
//					quickFetchMode == 0;
//				}
//			}
...
//
//	/*{
//		CLockGuard lock(&thread_mutex);
//		handleLocal = &lockFreeQueueH[handleIndex];
//		++handleIndex;
//	}
//
//	handleLocal->msgQueueHead = nullptr;
//	handleLocal->msgQueueTail = nullptr;
//	handleLocal->lock = 0;
//	handleLocal->queueSize = 0;*/
//	//if (sem_init(&read_sem, 0, 0) == -1)
//	//{
//	//	printf("func sem_init failed\n");
//	//}
//	//err = pthread_spin_init(&handleLocal->spin, 0);
//	//if (err != 0)
//	//{
//	//	printf("func pthread_spin_init failed\n");
//	//}
//
//	while (true)
//	{
//		while (readQueueHead == nullptr && EXIT == false)
//		{
//			if (sem_wait(&read_sem) == -1)
//			{
//				printf("In thread_handler, func sem_wait failed\n");
//			}
//		}
//		if (EXIT) break;
//
...
//#if (VER == MY_MALLOC)
//
//			msg = reinterpret_cast<MsgTag*>(psend - sizeof(MsgTag));
//			msg->next = nullptr;
//			select = index % sendThreads;
//
//			while (__sync_lock_test_and_set(&sendLOCK, 1))
//			{
//				usleep(0);
...
//			}
//
//			if (msgTailList[select] == nullptr)
//			{
//				msgHeadList[select] = msg;
//				msgTailList[select] = msg;
//			}
//			else
//			{
//				msgTailList[select]->next = msg;
//				msgTailList[select] = msg;
//			}
//			//++sendQueueSize;
//			//__sync_lock_release(&atomicLock[select]);
//			__sync_lock_release(&sendLOCK);
//#endif
//			//sem_post(&semList[select]);
//			//pthread_cond_signal(&send_cond); //唤醒一个等待该条件的线程
//
//#if (VER == MY_MALLOC)
//			mempool->TzFree(pread);
//			//free(pread - 8);
...
//	List<char*, TzMemPool<_SPN64K, _MEDIUM>>::iterator pos, pos2, posend;
//	int sem_value;
//	int i = 0;
//	int i3 = 0;
//
...
//		{
//			printf("In thread_send, func sem_wait failed\n");
//		}
//
//#if (VER == TC_MALLOC)
//		{
//			CLockGuard lock(&send_mutex);
//			//CLockGuardSpin lock(&send_spin);
//			//一般走到这里都表示需要处理数据收发了
//			if (!send_queue.empty())
//			{
//				pos = send_queue.begin();
//				posend = send_queue.end();
//
//				while (pos != posend)
//				{
//					psend = (*pos);
...
//						tm = 0;
//
//						/*std::map<pthread_t, thread_memmanage_t*>::iterator iter;
//						for (iter = ngx_mem_pool::__thread_cache.begin(); iter != ngx_mem_pool::__thread_cache.end(); ++iter)
//						{
//							thread_memmanage_t* memmanange = iter->second;
//							printf("alloc_size: %d ======== deposit_size: %d  tid %d\n", 
//								memmanange->_poolcapacity, memmanange->_pooldeposit_size, memmanange->_tid);
//						}*/
//						/*for (THREAD_CACHE_S* tc = mempool->__thread_heaps; tc; tc = tc->_next)
//						{
//							printf("alloc_size: %d ======== deposit_size: %d  tid %d\n",
//								TzMemPool<_SPN512K, _LARGE>::__capacity, tc->_Tdeposit_size, tc->_Tid);
//						}*/
//						/*for (THREAD_CACHE_S* tc = mempool->__thread_heaps; tc; tc = tc->_next)
//						{
//							printf("deposit_size: %d === tid %d\n",
//								tc->_Tdeposit_size1, tc->_Tid);
//						}*/
//
//						ISSTOP = true;
//					}
...
//				//posend = send_queue.end();
//
//				while (sendQueueHead != nullptr)
//				{
//					psend = reinterpret_cast<char*>(sendQueueHead) + sizeof(MsgTag);
//
//					sendQueueHead = sendQueueHead->next;
//
//					if (sendQueueHead == nullptr)
...
//#endif
//
////		while (send_queue.try_dequeue(psend))
////		{
////#if (VER == TC_MALLOC)
////			free(psend);
////#endif
...
////		}
//
//		sem_getvalue(&send_sem, &sem_value);
//		if (sem_value == 0 && sendQueueHead != nullptr)
//		{
//			sem_post(&send_sem);
//		}
//
//		if (i >= repeatTimes) break;
//	}
//
//	printf("The number of timer less than 65ms: %d and total time consuming: %f ms\n", i3, total_tm);
//	//usleep(500 * 1000);
